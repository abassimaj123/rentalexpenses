...
pw.Widget _buildReportPage(_ReportParams p) {
...
pw.Widget _buildComparisonPage(_ComparisonParams p) {
  final now = DateTime.now();
  final cur0 =
      NumberFormat.currency(locale: 'en_US', symbol: '\$', decimalDigits: 0);
  final dateFmt = DateFormat('MMMM d, yyyy');
...
pw.Widget _buildDepreciationPage(_DepreciationParams p) {
  final now = DateTime.now();
  final cur2 =
      NumberFormat.currency(locale: 'en_US', symbol: '\$', decimalDigits: 2);
  final cur0 =
...
Future<Uint8List> _buildComparisonPdf(_ComparisonParams p) async {
  ...
  pdf.addPage(pw.Page(
    pageFormat: PdfPageFormat.a4,
    margin: const pw.EdgeInsets.fromLTRB(36, 36, 36, 28),
    build: (_) => _buildComparisonPage(p),
  ));
  return pdf.save();
}
...
Future<Uint8List> _buildDepreciationPdf(_DepreciationParams p) async {
  ...
  pdf.addPage(pw.Page(
    pageFormat: PdfPageFormat.a4,
    margin: const pw.EdgeInsets.fromLTRB(36, 36, 36, 28),
    build: (_) => _buildDepreciationPage(p),
  ));
  return pdf.save();
}
